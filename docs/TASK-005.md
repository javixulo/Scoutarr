...
For `rename` mode, `mode` is `"rename"` and the rename has been applied on disk. Response also includes:
...
Feature: REST API — identify and rename movie
  ...
  Scenario: candidateIndex retries with selected candidate
    Given a POST request to /identify/movie with filename "Batman.mkv" and candidateIndex 3
    And Core returns a successful identification using the third candidate
    When the request is processed
    Then the response status is 200
    And the response contains mode "identify"

  Scenario: candidateIndex out of range returns 400
    Given a POST request to /identify/movie with filename "Batman.mkv" and candidateIndex 99
  ...
  Scenario: TMDB unreachable returns 500
    Given a POST request to /identify/movie with filename "The.Batman.2022.mkv"
  ...
  Scenario: No TMDB results returns 422
    Given a POST request to /identify/movie with filename "Zxqfnmvp.mkv"
  ...
  Scenario: Insufficient write permissions returns 403
    Given a POST request to /rename/movie with filename "The.Batman.2022.mkv"
    And the filesystem raises a permissions error
    When the request is processed
    Then the response status is 403
    And the response follows RFC 9457 format

  Scenario: Target filename already exists returns 409
    Given a POST request to /rename/movie with filename "The.Batman.2022.mkv"
    And a file named "The Batman (2022).mkv" already exists at the target path
    When the request is processed
